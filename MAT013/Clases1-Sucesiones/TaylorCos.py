...
def Taylor(x: float,n: int) -> float:
  return np.sum([(-1)**i*x**(2*i)/special.factorial(2*i) for i in range(n+1)])
...
def main():
  ...
  if Animado:
    ...
    def update(frame):
      ...
      ax.text(intervalo_x_fig[0],intervalo_y_fig[1],f'$n = {2*n}$', fontsize=Tam_fuentes, color='black',horizontalalignment='left',verticalalignment='center')
    ...
    ax.text(intervalo_x_fig[0],intervalo_y_fig[1],f'$n = {2*N_Max}$', fontsize=Tam_fuentes, color='black',horizontalalignment='left',verticalalignment='center')
  ...
  if Guardar_grafico:
    if Animado:
      ...
      archivo=f'{carpeta}/{Nombre}_{2*N_base}_{2*N_Max}.mp4'
      ...
      # Creo el nombre del archivo
      archivo=f'{carpeta}/{Nombre}_n{2*N_Max}.png'
