...
def main():
  ...
  if Mostrar_ejes & Ejes_clasicos:
    ...
    ax.set_xlabel(f'$x$',loc='right', fontsize=Font_size)
    ax.set_ylabel(f'$y$',loc='top',rotation='horizontal', fontsize=Font_size)
  ...
  plt.clabel(contours,inline=1,fontsize=Font_size,manual=[(1/2,1/np.sqrt(2)),(np.sqrt(2)/2,1),(np.sqrt(3)/2,np.sqrt(3)/np.sqrt(2)),(1,2/np.sqrt(2)),(np.sqrt(5)/2,np.sqrt(5)/np.sqrt(2))])
  # Cambiar tamaño de fuente de los ticks de los ejes
