...
def Draw_Sucesion_2D(n , intervalo_x = [0,6], intervalo_y = [0,1], Plot_dark = True, ocultar_numeros = False, ocultar_etiquetas = False, ocultar_funciones = False):
    ...
    ax.scatter(indices_suc, sucesion_a, color='cyan' if Plot_dark else 'blue', s=10, label=latex_tag_funcion_a)
    ax.scatter(indices_suc, sucesion_b, color='magenta' if Plot_dark else 'red', s=10, label=latex_tag_funcion_b)
    ax.scatter(indices_suc, sucesion_c, color='yellow' if Plot_dark else 'green', s=10, label=latex_tag_funcion_c)
    ...
    if not ocultar_funciones:
        ...
        ax.plot(x_continuo, func_b(x_continuo), color='magenta' if Plot_dark else 'red', linestyle=':')
        ax.plot(x_continuo, func_c(x_continuo), color='yellow' if Plot_dark else 'green', linestyle=':')
...
def main():
    ...
    st.set_page_config(page_title="Teorema del Sándwich para sucesiones", layout="wide", initial_sidebar_state='expanded', page_icon=':material/line_axis:')#, menu_items={'Get Help': 'https://www.extremelycoolapp.com/help','Report a bug': "https://www.extremelycoolapp.com/bug",'About': "# This is a header. This is an *extremely* cool app!"})
