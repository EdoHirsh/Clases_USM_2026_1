...
def Draw_Sucesion_1D(n , intervalo_x = [-0.05,1.05], intervalo_y = [-0.125,0.125],solo_ultimo = False, Plot_dark = True, ocultar_etiquetas = False):
    ...
    if not ocultar_etiquetas:
        if solo_ultimo:
            ...
            ax.plot([sucesion[-2],sucesion[-1]], [0.07, 0.07], color='cyan' if Plot_dark else 'blue')
...
def main():
    ...
    st.set_page_config(page_title='Ejemplo serie alternante', layout='wide', initial_sidebar_state='expanded', page_icon=':material/line_axis:')#, menu_items={'Get Help': 'https://www.extremelycoolapp.com/help','Report a bug': "https://www.extremelycoolapp.com/bug",'About': "# This is a header. This is an *extremely* cool app!"})
